...
/// <summary>
/// A persistence provider that stores objects in the user's configuration directory.
/// </summary>
/// <remarks>
/// Resolves to <c>$XDG_CONFIG_HOME/&lt;applicationName&gt;</c> when that variable is set to an absolute
/// path, and to <c>~/.config/&lt;applicationName&gt;</c> otherwise. The layout is the same on every
/// platform; on Windows <c>~</c> is <c>%USERPROFILE%</c>. Use this for user-editable settings. For
/// application state — caches, databases, saved documents — use the <c>DataHome</c> provider instead.
...
public sealed class ConfigHomePersistenceProvider<TKey>(
	...
	where TKey : notnull
{
	private readonly FileSystemPersistenceProvider<TKey> _inner = new(
		...
		UserDirectories.GetApplicationConfigDirectory(applicationName, subdirectory));
	...
	/// <inheritdoc/>
	public string ProviderName => "ConfigHome";
