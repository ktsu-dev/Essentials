...
public sealed class AppData<TKey> : IPersistenceProvider<TKey>
	where TKey : notnull
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="AppData{TKey}"/> class.
	/// </summary>
...
#pragma warning restore KTSU0003
	...
	private string GetAppDataDirectory(string applicationName, string? subdirectory)
	{
		...
		string basePath = _fileSystemProvider.Path.Combine(appDataPath, applicationName);

		if (!string.IsNullOrWhiteSpace(subdirectory))
		{
			basePath = _fileSystemProvider.Path.Combine(basePath, subdirectory);
		}
